import { DMANumber, IIRQ, IClose, ICloseData, ICPU, IAudio, IVideo, ARMMode, IIO, IDMA, IGBA, IGBAMMU, MemoryRegion, IBIOS, IMemoryView, OpExecMode, ILog, MemoryRegionSize } from "../interfaces.ts";
...
export default class GameBoyAdvanceInterruptHandler implements IIRQ, IClose {
    ...
    private getGBAMMU(): IGBAMMU {
        return this.getGBA().mmu as IGBAMMU;
    }

    private getBIOS(): IBIOS {
        return this.getGBAMMU().bios as IBIOS;
    }
    ...
    swi(opcode: number): void {
        ...
        switch (opcode) {
            ...
            case 0x01:
                ...
                if (regions & 0x1C) {
                    this.getVideo().renderPath.clearSubsets(MemoryRegionSize.PALETTE_RAM /* can remove */, regions);
                }
                if (regions & 0xE0) {
