import { IGPIO, IGBARTC, ILog } from "../interfaces.ts";

export default class GameBoyAdvanceRTC implements IGBARTC {
    gpio: IGPIO
    pins: number
    direction: number
    totalBytes: number[]
        ...
        switch (this.command) {
            case -1:
                if ((this.bits & 0x0F) == 0x06) {
                    ...
                    (this.gpio.core as ILog).WARN('Invalid RTC command byte: ' + this.bits.toString(16));
                }
                break;
            case 4:
                // Control
